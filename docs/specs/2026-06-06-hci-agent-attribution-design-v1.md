...
### 3.1 素材：20 對 contrastive pair

忠於 proposal 的 20 trial 與四類 pair。xAI ground-truth 實際先標了 3 類（harness 主效應、model 主效應、交互，共 20），**無 noise**；為完整呈現 proposal 的四選一與 RQ2 混淆矩陣，補入 4 對既有正式重跑的 noise pair，並從非 noise pair 中確定性修剪 4 對，避免把研究擴成 proposal 以外的 24 題。

| Pair 類型 | 數量 | 配對方式 | 正解（ground truth） | 資料來源 |
|---|---|---|---|---|
| Harness-only | 5 | 同 model、不同 harness | harness 主效應 | `analysis/phase3/hci-ground-truth-labels.json` |
| Model-only | 5 | 同 harness、不同 model | model 主效應 | 同上 |
| Interaction | 6 | factorial 顯著異常組合 | 交互 | 同上 |
...
情境為受試者所見的情境框（cover story），資料無此欄位，由我們依 task 性質與 quota 分派，分派表寫入 `pairs.json` 並在報告交代規則。4 對 noise 併入既有三情境（不另立情境），最後配比固定為 8/8/4。
...
**受試者內設計（within-subject）＋對半平衡**：20 對拆為 Set1／Set2 各 10 對（四類型與三情境在兩 set 盡量等比）。每位受試者前 10 題用一個條件、後 10 題用另一個條件（不同題，避免記憶污染）。跨受試者輪流哪個 set 配 A、哪個配 B，平衡題目難度混淆。
...
| 元件 | 位置／技術 | 職責 |
|---|---|---|
| 資料準備 `prep/build_pairs.py` | VPS、Python | 讀 xAI `hci-ground-truth-labels.json`＋`analysis/phase4/metrics-summary.json`，並從 raw traces 配 4 對 noise，輸出自足的 `data/pairs.json`（20 對：A 內容、B 內容、正解、情境、成敗率） |
| pair-viewer `viewer/` | 靜態單頁（index.html＋app.js＋styles.css） | 載入 `data/pairs.json`，跑 20 題協定，作答存成可下載 `responses-<participant>.json`。無後端、可在 Mac 本機開，便於截圖與錄影 |
